...
inline __device__ void WoBFunc(const unsigned int idx, const DeviceArgs &args) {
    ...
    wob::DomainPoint<ScalarType, Dim> x{wob::utils::idx_to_domain_point<ScalarType, Dim>(idx, args.grid_res, 2.0)};
...
inline __device__ void WoSFunc(const unsigned int idx, const DeviceArgs &args) {
    ...
    Eigen::Matrix<ScalarType, Dim, 1> _x = wob::utils::idx_to_domain_point<ScalarType, Dim>(idx, args.grid_res, 2.0);
...
int main(int argc, char *argv[]) {
    ...
    for (size_t f = 0; f < F.rows(); f++) {
        ...
        ScalarType boundary_value = (v1.x() < -0.9f && v2.x() < 0.9f) ? 1.0f : -1.0f;
        elems.push_back({v1, v2, n, boundary_value, boundary_value, wob::Neumann});
    ...
    if (evaluate_at_all_points)
        ...
        for (size_t i = 0; i < num_evaluation_points; i++) {
            Eigen::Matrix<ScalarType, Dim, 1> x = wob::utils::idx_to_domain_point<ScalarType, Dim>(i, grid_res, 2.0);
